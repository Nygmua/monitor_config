#!/usr/bin/env bash
# bash <(curl -Lso- https://cf.j1ang.eu.org/sh/ip46.sh)
...
check_root() {
  [[ "$USER" == 'root' ]] || { danger "Please run this script as root!"; exit 1; }
}
...
restore_ip46() {
  ...
  if [[ "$@" = 'info' ]]; then restore_success;check_ip46; fi
}
prefer_ipv4() {
  restore_ip46
...
restore_ipv6() {
  ...
  if [[ "$@" = 'info' ]]; then reload_sysctl;restart_network;restore_success;check_ipv6; fi
}
...
main() {
  if [[ "$num" == "1" ]]; then prefer_ipv4
  elif [[ "$num" == "2" ]]; then prefer_ipv6
  elif [[ "$num" == "3" ]]; then restore_ip46 'info'
  elif [[ "$num" == "4" ]]; then enable_ipv6
  elif [[ "$num" == "5" ]]; then disable_ipv6
  elif [[ "$num" == "6" ]]; then restore_ipv6 'info'
  else exit
  fi
}
